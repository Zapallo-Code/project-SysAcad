...
class TestUniversitySerializerValidation(unittest.TestCase):
    ...
    def test_name_with_leading_spaces(self, mock_serializer):
        """Test name with leading spaces."""
        from app.serializers import UniversitySerializer
        
        serializer = UniversitySerializer(data={'name': '  UNC', 'acronym': 'UNC'})
        mock_serializer.return_value.is_valid.return_value = False
        mock_serializer.return_value.errors = {
            'name': ['Name cannot start with whitespace.']
        }
        
        self.assertFalse(serializer.is_valid())

    @patch('app.serializers.university.UniversitySerializer')
    def test_name_with_trailing_spaces(self, mock_serializer):
        ...
        mock_serializer.return_value.is_valid.return_value = False
        mock_serializer.return_value.errors = {
            'name': ['Name cannot end with whitespace.']
        }
        
        self.assertFalse(serializer.is_valid())

    @patch('app.serializers.university.UniversitySerializer')
    def test_acronym_lowercase(self, mock_serializer):
        """Test acronym with lowercase letters."""
        from app.serializers import UniversitySerializer
        
        serializer = UniversitySerializer(data={'name': 'UNC', 'acronym': 'unc'})
        mock_serializer.return_value.is_valid.return_value = False
        mock_serializer.return_value.errors = {
            'acronym': ['Acronym must be uppercase.']
        }
...
class TestSubjectSerializerValidation(unittest.TestCase):
    ...
    @patch('app.serializers.subject.SubjectSerializer')
    def test_hours_zero(self, mock_serializer):
    ...
    @patch('app.serializers.subject.SubjectSerializer')
    def test_hours_too_large(self, mock_serializer):
        """Test subject with too many hours."""
        from app.serializers import SubjectSerializer
        ...
        mock_serializer.return_value.is_valid.return_value = False
        mock_serializer.return_value.errors = {
            'hours': ['Hours cannot exceed 2000.']
        }
...
class TestFacultySerializerValidation(unittest.TestCase):
    ...
    def test_name_only_special_chars(self, mock_serializer):
        ...
        mock_serializer.return_value.is_valid.return_value = False
        mock_serializer.return_value.errors = {
            'name': ['Name must contain letters.']
        }
