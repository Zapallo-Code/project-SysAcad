...
class TestStudentServiceErrors(unittest.TestCase):
    ...
    def test_create_duplicate_student_number_error(self, mock_repo):
        ...
        self.assertIn('student number', str(context.exception).lower())
...
class TestRepositoryErrors(unittest.TestCase):
    ...
    def test_multiple_objects_returned(self, mock_model):
        ...
        mock_model.objects.get.side_effect = MultipleObjectsReturned()
        
        with self.assertRaises(MultipleObjectsReturned):
            StudentRepository.find_by_id(1)
...
class TestTransactionErrors(unittest.TestCase):
    ...
    @patch('app.services.university.UniversityRepository')
    def test_transaction_atomic_rollback(self, mock_repo, mock_transaction):
        ...
        mock_repo.create.side_effect = Exception('Error during creation')
        
        with self.assertRaises(Exception):
            with mock_transaction.atomic():
                UniversityService.create({'name': 'Test'})
