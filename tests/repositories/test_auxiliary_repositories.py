...
class TestDedicationTypeRepository(unittest.TestCase):
    ...
    def test_create_success(self, mock_model):
        ...
        mock_instance = self.mock_dedication
        mock_model.return_value = mock_instance

        result = DedicationTypeRepository.create(
            {"name": "Dedicación Exclusiva", "hours": 40}
        )

        mock_model.assert_called_once_with(name="Dedicación Exclusiva", hours=40)
...
class TestPositionCategoryRepository(unittest.TestCase):
    ...
    def test_create_success(self, mock_model):
        ...
        mock_instance = self.mock_category
        mock_model.return_value = mock_instance

        result = PositionCategoryRepository.create({"name": "Profesor Titular"})

        mock_model.assert_called_once_with(name="Profesor Titular")
        mock_instance.full_clean.assert_called_once()
        mock_instance.save.assert_called_once()
...
class TestSpecialtyTypeRepository(unittest.TestCase):
    ...
    def test_create_success(self, mock_model):
        ...
        mock_instance = self.mock_specialty_type
        mock_model.return_value = mock_instance
