...
class OrientationRepository:
    ...
    def find_by_id(id: int) -> Optional[Orientation]:
        try:
            return Orientation.objects.select_related('specialty', 'plan', 'subject').get(id=id)
        except ObjectDoesNotExist:
            return None

    @staticmethod
    def find_all() -> List[Orientation]:
        return list(Orientation.objects.select_related('specialty', 'plan', 'subject').all())

    @staticmethod
    def find_by_specialty(specialty_id: int) -> List[Orientation]:
        return list(
            Orientation.objects.filter(specialty_id=specialty_id).select_related('specialty', 'plan', 'subject')
        )

    @staticmethod
    def find_by_plan(plan_id: int) -> List[Orientation]:
        return list(Orientation.objects.filter(plan_id=plan_id).select_related('specialty', 'plan', 'subject'))

    @staticmethod
    def find_by_subject(subject_id: int) -> List[Orientation]:
        return list(Orientation.objects.filter(subject_id=subject_id).select_related('specialty', 'plan', 'subject'))

    @staticmethod
    def find_by_name_specialty_plan(name: str, specialty_id: int, plan_id: int) -> Optional[Orientation]:
        try:
    ...
    @staticmethod
    def exists_by_name_specialty_plan(name: str, specialty_id: int, plan_id: int) -> bool:
