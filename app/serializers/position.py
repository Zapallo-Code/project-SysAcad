...
class PositionSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if not value or value.strip() == '':
            raise serializers.ValidationError('Position name cannot be only whitespace.')
    ...
    def validate_position_category_id(self, value):
        if value <= 0:
            raise serializers.ValidationError('Position category ID must be a positive integer.')

        return value

    def validate_dedication_type_id(self, value):
        if value <= 0:
            raise serializers.ValidationError('Dedication type ID must be a positive integer.')
