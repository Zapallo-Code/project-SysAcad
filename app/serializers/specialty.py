...
class SpecialtySerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if not value or value.strip() == '':
            raise serializers.ValidationError('Specialty name cannot be only whitespace.')

        if not value[0].isalpha():
            raise serializers.ValidationError('Specialty name must start with a letter.')
    ...
    def validate_specialty_type_id(self, value):
        if value <= 0:
            raise serializers.ValidationError('Specialty type ID must be a positive integer.')
    ...
    class Meta:
        model = Specialty
        fields = ['id', 'name', 'letter', 'observation', 'specialty_type_id', 'faculty_id',
