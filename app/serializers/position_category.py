...
class PositionCategorySerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if not value or value.strip() == '':
            raise serializers.ValidationError('Position category name cannot be only whitespace.')

        if not value[0].isalpha():
            raise serializers.ValidationError('Position category name must start with a letter.')
