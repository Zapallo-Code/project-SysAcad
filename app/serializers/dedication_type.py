...
class DedicationTypeSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if not value or value.strip() == '':
            raise serializers.ValidationError('Dedication type name cannot be only whitespace.')

        if not value[0].isalpha():
            raise serializers.ValidationError('Dedication type name must start with a letter.')
