...
class StudentService:
    ...
    def create(student_data: dict) -> Any:
        logger.info(f"Creating student: {student_data.get('first_name')} {student_data.get('last_name')}")
        ...
        if StudentRepository.exists_by_student_number(student_data.get('student_number')):
            ...
            raise ValueError(f"Student number {student_data.get('student_number')} is already taken")

        if StudentRepository.exists_by_document_number(student_data.get('document_number')):
            logger.error(f"Document number {student_data.get('document_number')} already registered")
            raise ValueError(f"Document number {student_data.get('document_number')} is already registered")

        if not SpecialtyRepository.exists_by_id(student_data.get('specialty_id')):
            logger.error(f"Specialty with id {student_data.get('specialty_id')} not found")
            raise ValueError(f"Specialty with id {student_data.get('specialty_id')} does not exist")
        ...
        if not DocumentTypeRepository.exists_by_id(student_data.get('document_type_id')):
            ...
            raise ValueError(f"Document type with id {student_data.get('document_type_id')} does not exist")
    ...
    @staticmethod
    def is_enrollment_valid(birth_date: datetime.date, enrollment_date: datetime.date) -> bool:
